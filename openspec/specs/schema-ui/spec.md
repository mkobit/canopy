# schema-ui Specification

## Purpose
Give `apps/web` a "Schema" section so namespaces, NodeTypes, EdgeTypes, and PropertyTypes can be authored through the UI instead of only via direct `@canopy/graph` op calls.
Depends on the `type-authoring` capability for the underlying data model and create ops.
