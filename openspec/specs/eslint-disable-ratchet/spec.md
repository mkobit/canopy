...
Keep `eslint-disable` escape hatches from accreting as the codebase eliminates them.
Two guards run in `bun run lint`: unused directives are rejected, and the total directive count may only decrease against a committed baseline — making elimination a ratchet rather than a one-time cleanup.
