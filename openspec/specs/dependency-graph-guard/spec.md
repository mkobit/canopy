# dependency-graph-guard Specification

## Purpose

A lint-wired check that derives the internal `@canopy/*` package dependency graph from workspace manifests and enforces the kernel-leaf invariant, acyclicity (including dev edges), and documentation parity with `docs/architecture/bounded-contexts.md`, so architectural invariant #1 and the hand-maintained dependency diagram cannot drift from reality without failing `bun run lint`.
