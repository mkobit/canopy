# agent-daemon-host Specification

## Purpose

A dedicated foreground host process (`apps/daemon`) that stands up the `@canopy/api-adapter` IPC server against a live, loaded `GraphSession` — the dockerd-style server counterpart to the `apps/cli` client — so an agent can reach a running graph without hand-wiring the socket itself.
