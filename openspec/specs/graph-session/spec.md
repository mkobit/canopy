# graph-session Specification

## Purpose
The event-sourced write path in `@canopy/graph`: session lifecycle (load/project, commit, subscribe) and convergent incremental projection with the permutation-invariance guarantee, so every consumer of an `EventLogStore` shares one write path instead of mutating graph state directly.
