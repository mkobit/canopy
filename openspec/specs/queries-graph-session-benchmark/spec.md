# queries-graph-session-benchmark Specification

## Purpose

A non-gating benchmark script that measures `@canopy/queries` query execution and `GraphSession` fold/re-projection latency under a 10,000+ node, 20,000+ edge graph, so perf-sensitive changes to the query engine or projection path have real median/p95 numbers to check against without making `bun test` depend on wall-clock timing.
