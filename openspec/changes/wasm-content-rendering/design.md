...
## Implementation notes (settled during build)

- **Tier-1 executes the app-bundled transpiled artifact, not the stored `wasm_binary`.** Running a componentized `.wasm` in the browser requires the jco-transpiled JS + core modules + WASI shim; there is no runtime component transpiler here (deferred with Tier 2). So the resolver dispatches to the build-time-transpiled component (bundled via Vite, code-split behind a dynamic import), and bootstrap seeds the Plugin node with an **empty `wasm_binary`** placeholder rather than embedding a multi-megabyte web-app build artifact into the kernel. The capability/binding/sanitization path is still fully exercised through `executeSandboxedGuestPlugin`.
- **The dogfood revealed a real host-binding gap (as predicted).** `executeSandboxedGuestPlugin` and `createWasmHostBindings` used Node's `Buffer.byteLength`, which is undefined in the browser; both were made isomorphic via `TextEncoder`. This is the first browser use of the executor and exactly the kind of gap decision 3 anticipated.
- **In-browser instantiation proven.** The StarlingMonkey/WASI component instantiates and renders in a real browser via `@bytecodealliance/preview2-shim`'s `WASIShim`; verified by the Playwright e2e (`data-render-status="ok"` on a closed shadow root, hostile `<script>` never executes).
- **Sanitizer unit tests cover the pure CSS policy only.** happy-dom's DOMParser mangles DOMPurify's tree, so element/attribute stripping is verified end-to-end in the browser e2e; the CSS-policy string logic is unit-tested directly.
- **Incidental fix:** removed a dead `packages/sync` reference from `apps/web/tsconfig.build.json` (the package was deleted long ago) that blocked Playwright's strict tsconfig loader from running any e2e.
