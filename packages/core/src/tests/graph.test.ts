...
describe('Core Graph Engine', () => {
  ...
  it('should create a graph with bootstrap nodes', () => {
    ...
    // 19 nodes total (6 Defs + 3 Core Node Types + 4 Core Edge Types + 3 Queries + 3 Views)
    expect(emptyGraph.nodes.size).toBe(19);
  ...
  it('should add nodes immutably and emit events', () => {
    ...
    expect(emptyGraph.nodes.size).toBe(19); // Original unmodified (bootstrap nodes)
    ...
    expect(g1.nodes.size).toBe(20); // Previous version unmodified
  ...
  it('should remove nodes and connected edges and emit events', () => {
    ...
    expect(g.nodes.size).toBe(21); // 19 bootstrap + 2 nodes
    expect(g.edges.size).toBe(1);
