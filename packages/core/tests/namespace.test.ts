...
import { resolveNamespace, getNodesInNamespace } from '../src/namespace';
...
describe('namespace utilities', () => {
  it('should find nodes by namespace', () => {
    ...
    graph = unwrap(addNode(graph, importedType, { deviceId: asDeviceId('00000000-0000-0000-0000-000000000000') })).graph;
    graph = unwrap(addNode(graph, normalType, { deviceId: asDeviceId('00000000-0000-0000-0000-000000000000') })).graph;

    // Node 1: uses imported type, no override -> imported
    const node1 = createNode();
    node1.type = asTypeId('type-imported');
    graph = unwrap(addNode(graph, node1, { deviceId: asDeviceId('00000000-0000-0000-0000-000000000000') })).graph;
    ...
    graph = unwrap(addNode(graph, node2, { deviceId: asDeviceId('00000000-0000-0000-0000-000000000000') })).graph;

    // Node 3: uses normal type, overrides to 'imported'
    const node3 = createNode({ namespace: 'imported' });
    node3.type = asTypeId('type-normal');
    graph = unwrap(addNode(graph, node3, { deviceId: asDeviceId('00000000-0000-0000-0000-000000000000') })).graph;

    // Node 4: uses normal type, no override -> user
    const node4 = createNode();
    node4.type = asTypeId('type-normal');
    graph = unwrap(addNode(graph, node4, { deviceId: asDeviceId('00000000-0000-0000-0000-000000000000') })).graph;
    ...
    expect(importedNodes.map(n => n.id).sort()).toEqual([importedType.id, node1.id, node3.id].sort());

    const userSettingsNodes = getNodesInNamespace(graph, 'user-settings').filter(n => [importedType.id, normalType.id, node1.id, node2.id, node3.id, node4.id].includes(n.id));
    expect(userSettingsNodes.length).toBe(1);
    expect(userSettingsNodes[0].id).toBe(node2.id);

    const userNodes = getNodesInNamespace(graph, 'user').filter(n => [importedType.id, normalType.id, node1.id, node2.id, node3.id, node4.id].includes(n.id));
    expect(userNodes.length).toBe(1); // node4 defaults to user
    expect(userNodes[0].id).toBe(node4.id);

    const systemNodes = getNodesInNamespace(graph, 'system').filter(n => [importedType.id, normalType.id, node1.id, node2.id, node3.id, node4.id].includes(n.id));
