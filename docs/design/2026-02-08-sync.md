...
> **Resolution (2026-07-03)**: the divergence flagged here is decided — this document's no-CRDT event-replication model stands, and the Yjs-based `@canopy/sync` is being removed entirely (including the block editor's `Y.Text`; text becomes block-level property updates).
> See [2026-07-03-event-log-storage-and-sync.md](2026-07-03-event-log-storage-and-sync.md) for the reconciliation design; it also answers open questions 1–3 below for the file-based transport (per-device JSONL segment folders, watermark reconciliation, batches never span segments).
> Implementation tracking: openspec change `event-log-source-of-truth` (beads `canopy-1q5.5`–`.8`); file transport is bead `canopy-1q5.3`/`.4`.
...
It delivers events; the projection layer resolves conflicts using the rules defined in the [event system](2026-02-08-event-system.md), section 6.
...
| Conflict resolution (LWW, delete-wins) | [Event system](2026-02-08-event-system.md)                                   |
| Storage engine persistence             | [Storage layer](2026-02-08-storage-layer.md)                                 |
| Real-time collaborative text editing   | Not a current goal                                                           |
