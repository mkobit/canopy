...
> **Implementation note (2026-07-03)**: the shipped sync engine (`@canopy/sync`) is built on Yjs (`Y.Doc`, CRDT merge), which section 7 of this document explicitly rules out.
> Only an in-memory same-process test provider exists; no real transport (file-based/WebSocket/HTTP/LAN) is wired up yet.
> Tracked in bead `canopy-1q5` (epic) / `canopy-1q5.1` (the reconciliation decision, needs a dedicated design session).
> Do not build new transports against this document's protocol until that decision lands.
